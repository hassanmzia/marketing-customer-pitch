    ...
    def export(self, request, pk=None):
        ...
        if format_type == 'pdf':
            ...
            title_style = ParagraphStyle(
                'PitchTitle', parent=styles['Heading1'],
                fontSize=20, spaceAfter=6, textColor=colors.HexColor('#111827'),
            )
            ...
            body_style = ParagraphStyle(
                ...
                fontSize=11, leading=16, textColor=colors.HexColor('#1f2937'),
            )
            ...
            elements = []
            elements.append(Paragraph(pitch.title or 'Pitch', title_style))
            meta = (
                f"<b>Customer:</b> {pitch.customer.name} ({pitch.customer.company}) &nbsp;&nbsp; "
                f"<b>Type:</b> {pitch.get_pitch_type_display()} &nbsp;&nbsp; "
                f"<b>Tone:</b> {pitch.get_tone_display()} &nbsp;&nbsp; "
                f"<b>Version:</b> v{pitch.version}"
            )
            elements.append(Paragraph(meta, meta_style))
            elements.append(Spacer(1, 8))

            # Convert markdown content to simple HTML paragraphs for reportlab
            content_html = md.markdown(pitch.content or '', extensions=['extra', 'nl2br'])
            # reportlab Paragraph supports a subset of HTML
            for para_text in content_html.split('\n'):
                ...
                if para_text:
                    try:
                        ...
                        # If reportlab can't parse a fragment, add as plain text
                        elements.append(Paragraph(para_text.replace('<', '&lt;'), body_style))
                        elements.append(Spacer(1, 4))
            ...
            if pitch.scores:
                ...
                score_title = ParagraphStyle(
                    'ScoreTitle', parent=styles['Heading2'],
                    fontSize=14, textColor=colors.HexColor('#4f46e5'), spaceAfter=8,
                )
                elements.append(Paragraph('Scores', score_title))
                table_data = [['Dimension', 'Score']]
                for dim, score_val in pitch.scores.items():
                    table_data.append([dim.title(), f'{score_val * 100:.0f}%'])
                avg = pitch.average_score
                if avg is not None:
                    table_data.append(['Overall', f'{avg * 100:.0f}%'])
            ...
            elements.append(Paragraph(
                f"Generated by {pitch.generated_by} &bull; {pitch.created_at.strftime('%B %d, %Y %I:%M %p')}",
                footer_style,
            ))
