...
export default function TemplateLibrary() {
  ...
  const [newTemplate, setNewTemplate] = useState({ name: '', description: '', content: '', industry: '', pitch_type: 'initial' });
        ...
        <Modal isOpen={!!previewTemplate} onClose={() => setPreviewTemplate(null)} title={previewTemplate.name}>
          <div className="space-y-4">
            ...
            <div className="flex gap-2">
              {previewTemplate.industry && <span className="rounded-full bg-blue-50 px-2.5 py-0.5 text-xs font-medium text-blue-700 dark:bg-blue-900/20 dark:text-blue-400">{previewTemplate.industry}</span>}
            ...
            <div className="max-h-80 overflow-y-auto rounded-lg border border-gray-200 p-4 dark:border-gray-700">
              <div className="prose prose-sm max-w-none dark:prose-invert">
                <ReactMarkdown>{previewTemplate.content ?? 'No content'}</ReactMarkdown>
              </div>
            </div>
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <select value={newTemplate.industry} onChange={(e) => setNewTemplate((prev) => ({ ...prev, industry: e.target.value }))} className="w-full rounded-lg border border-gray-200 bg-white px-3 py-2.5 text-sm focus:border-indigo-500 focus:outline-none dark:border-gray-700 dark:bg-gray-800 dark:text-white">
            ...
            <div>
              ...
              <textarea value={newTemplate.content} onChange={(e) => setNewTemplate((prev) => ({ ...prev, content: e.target.value }))} rows={8} placeholder="Write your pitch template content using Markdown..." className="w-full rounded-lg border border-gray-200 bg-white px-4 py-2.5 font-mono text-sm focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500 dark:border-gray-700 dark:bg-gray-800 dark:text-white" />
            </div>
            <div className="flex justify-end gap-3">
